declare module "react-native-fetch-blob" {
    ...
    interface RNFetchBlobStatic {
        fetch(method: Methods, url: string, headers: { [key: string]: string }, body: any): StatefulPromise<RNFetchBlobResponse>;
    ...
    class PolyfillFetch extends RNFetchBlobFetchPolyfill {
    ...
    class PolyfillBlob extends EventTarget {
        /**
    ...
    interface PolyfillFile extends PolyfillBlob {
    }
    ...
    namespace PolyfillXMLHttpRequest {
    ...
    interface FS {
         ...
         * @return {Promise}
         */
        scanFile(pairs: { [key: string]: string }[]): Promise<void>;
    ...
    /**
     * A class inherits Promise, it has extra method like progress, uploadProgress,
    ...
    interface StatefulPromise<T> extends Promise<T> {
         ...
         */
        cancel(): Promise<RNFetchBlobResponse>;

        /**
         * Add an event listener which triggers when data receiving from server.
         */
        progress(callback: (received: number, total: number) => void): Promise<RNFetchBlobResponse>;

        /**
         * Add an event listener with custom configuration
         */
        progress(config: { count?: number, interval?: number }, callback: (received: number, total: number) => void): Promise<RNFetchBlobResponse>;

        /**
         * Add an event listener with custom configuration.
         */
        uploadProgress(callback: (sent: number, total: number) => void): Promise<RNFetchBlobResponse>;

        /**
         * Add an event listener with custom configuration
         */
        uploadProgress(config: { count?: number, interval?: number }, callback: (sent: number, total: number) => void): Promise<RNFetchBlobResponse>;
         ...
         * app become active.
         */
        expire(callback: () => void): Promise<void>;
    }

    /**
     * When fetch success, it resolve a FetchBlobResponse object as first argument.
     * FetchBlobResponse object has the following methods
     * (these method are synchronous, so you might take quite a performance impact if the file is big)
     */
    interface RNFetchBlobResponse {
         ...
         */
        readStream(encoding: string, bufferSize: number): Promise<any>;

        /**
         * when the response data is cached in a file, this method adds the file into the session.
    ...
    interface AddAndroidDownloads {
        ...
        mime: string;
        /**
         * A boolean value, see Officail Document (https://developer.android.com/reference/android/app/DownloadManager.html#addCompletedDownload(java.lang.String, java.lang.String, boolean, java.lang.String, java.lang.String, long, boolean))
         */
